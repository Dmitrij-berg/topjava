...
//        .toLocalTime();
    ...
    public static List<UserMealWithExceed>  getFilteredMealsWithExceededByCycle(List<UserMeal> mealList, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map <LocalDate, Integer> calloriesSumPerDate = new HashMap<>();
        for (UserMeal meal : mealList){
           LocalDate mealDate = meal.getDateTime().toLocalDate();
            calloriesSumPerDate.put(mealDate, calloriesSumPerDate.getOrDefault(mealDate, 0) + meal.getCalories());
        }
        ...
        for (UserMeal meal:mealList) {
            ...
            if (TimeUtil.isBetween(dateTime.toLocalTime(), startTime, endTime)){
                mealExceeded.add(new UserMealWithExceed(dateTime, meal.getDescription(),
                        meal.getCalories(), calloriesSumPerDate.get(dateTime.toLocalDate()) > caloriesPerDay));
            }
        }
